...
class muresolution
{
  ...
  public:
    ...
    double kSmear(double pt, double eta, int nlayers = 0, TYPE type = Extra)
    {
        ...
        while (1 + x <= 0) {
            ...
            if (type == Extra) {
                ...
                if (RD > RM)
                    x = sqrt(RD * RD - RM * RM) * cb[H][F].invcdf(u);
                else
                    x = 0;
            } else if (type == Data)
                x = kDat[H] * Sigma(pt, H, F) * cb[H][F].invcdf(u);
            else
                x = Sigma(pt, H, F) * cb[H][F].invcdf(u);
            // else		    x = kRes[H]*Sigma(pt, H, F)*cb[H][F].invcdf(u); //for next
            // iteration
        }
    ...
    const double BETA[13] = {
        0.00, 0.20, 0.40, 0.60, 0.80, 1.00, 1.20, 1.40, 1.60, 1.85, 2.10, 2.25, 2.40};
...
// const double muresolution::kRes[12] = {0.98958, 0.98931, 0.98564, 1.00798, 0.99354, 0.99735,
// 0.99735, 1.00192, 1.00584, 1.00065, 0.99354, 0.99498};
// const double muresolution::kDat[12] = {1.00239, 1.01823, 1.03872, 1.04986, 1.05237, 1.08296,
// 1.08108, 1.11542, 1.12020, 1.11069, 1.07470, 1.10859};
