        ...
        if (it == m_values.cend()) {
            throwException(edm::errors::NotFound,
                           "JER parametrisation depends on '" +
                               ...
                               "' but no value for this parameter has been specified. Please call "
                               "the appropriate 'set' function of the JME::JetParameters object");
        }
...
JetResolutionObject::Definition::Definition(const std::string &definition)
{
    ...
    std::transform(
        formula_str_lower.begin(), formula_str_lower.end(), formula_str_lower.begin(), ::tolower);

    if (formula_str_lower == "none") m_formula_str = "";
...
void JetResolutionObject::Definition::init()
{
    ...
    for (const auto &bin : m_bins_name) {
        ...
        if (b == JetParameters::binning_to_string.right.cend()) {
            throwException(edm::errors::UnimplementedFeature,
                           "Bin name not supported: '" + bin + "'");
        }
...
JetResolutionObject::Record::Record(const std::string &line, const Definition &def)
{
    ...
    if (tokens.size() <
        (def.nBins() * 2 + def.nVariables() * 2 + 1 + (n_parameters - def.nVariables() * 2))) {
        throwException(edm::errors::ConfigFileReadError,
                       "Invalid record. Please check file format. Record: " + line);
    }
...
void JetResolutionObject::dump() const
{
    ...
    for (const auto &record : m_records) {
        ...
        for (const auto &bin : record.getBinsRange()) {
            std::cout << "        " << m_definition.getBinName(index) << " [" << bin.min << " - "
                      << bin.max << "]" << std::endl;
            index++;
        }
        ...
        for (const auto &r : record.getVariablesRange()) {
            std::cout << "        " << m_definition.getVariableName(index) << " [" << r.min << " - "
                      << r.max << "] " << std::endl;
            index++;
        }
...
void JetResolutionObject::saveToFile(const std::string &file) const
{
    ...
    fout << "    "
         << (m_definition.getFormulaString().empty() ? "None" : m_definition.getFormulaString())
         << "    Resolution}" << std::endl;
...
const JetResolutionObject::Record *
JetResolutionObject::getRecord(const JetParameters &bins_parameters) const
{
    ...
    for (const auto &record : m_records) {
        ...
        for (const auto &bin : record.getBinsRange()) {
            if (bin.is_inside(bins[current_bin])) valid_bins++;
                                           ...
                                           const JetParameters &variables_parameters) const
{
    ...
    for (size_t index = 0; index < m_definition.nVariables(); index++) {
        variables_[index] = clip(variables[index],
                                 record.getVariablesRange()[index].min,
