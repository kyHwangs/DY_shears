...
JetCorrectorParameters::Definitions::Definitions(const std::vector<std::string> &fBinVar,
                                                 ...
                                                 bool fIsResponse)
{
    for (unsigned i = 0; i < fBinVar.size(); i++) mBinVar.push_back(fBinVar[i]);
    for (unsigned i = 0; i < fParVar.size(); i++) mParVar.push_back(fParVar[i]);
    mFormula = fFormula;
...
JetCorrectorParameters::Definitions::Definitions(const std::string &fLine)
{
    ...
    if (!tokens.empty()) {
        ...
        for (unsigned i = 0; i < nvar; i++) mBinVar.push_back(tokens[i + 1]);
        for (unsigned i = 0; i < npar; i++) mParVar.push_back(tokens[nvar + 2 + i]);
    ...
    while (std::getline(input, line)) {
        ...
        if (currentSection == fSection) {
            ...
            for (unsigned i = 0; i < mDefinitions.nBinVar(); ++i)
                if (record.xMin(i) == 0 && record.xMax(i) == 0) check = false;
            if (record.nParameters() == 0) check = false;
            if (check) mRecords.push_back(record);
        }
    }
    if (currentDefinitions == "") handleError("JetCorrectorParameters", "No definitions found!!!");
...
int JetCorrectorParameters::binIndex(const std::vector<float> &fX) const
{
    ...
    for (unsigned i = 0; i < size(); ++i) {
        ...
        for (unsigned j = 0; j < N; j++)
            if (fX[j] >= record(i).xMin(j) && fX[j] < record(i).xMax(j)) tmp += 1;
        if (tmp == N) {
            result = i;
            break;
        }
...
int JetCorrectorParameters::neighbourBin(unsigned fIndex, unsigned fVar, bool fNext) const
{
    ...
    for (unsigned i = 0; i < size(); ++i) {
        ...
        for (unsigned j = 0; j < fVar; j++)
            if (fabs(record(i).xMin(j) - record(fIndex).xMin(j)) < 0.0001) tmp += 1;
        for (unsigned j = fVar + 1; j < N; j++)
            if (fabs(record(i).xMin(j) - record(fIndex).xMin(j)) < 0.0001) tmp += 1;
        if (tmp < N - 1) continue;
        if (tmp == N - 1) {
            if (fNext)
                if (fabs(record(i).xMin(fVar) - record(fIndex).xMax(fVar)) < 0.0001) tmp += 1;
            if (!fNext)
                if (fabs(record(i).xMax(fVar) - record(fIndex).xMin(fVar)) < 0.0001) tmp += 1;
        }
        if (tmp == N) {
            result = i;
            break;
...
std::vector<float> JetCorrectorParameters::binCenters(unsigned fVar) const
{
    ...
    for (unsigned i = 0; i < size(); ++i) result.push_back(record(i).xMiddle(fVar));
    return result;
}
//------------------------------------------------------------------------
...
void JetCorrectorParameters::printFile(const std::string &fFileName) const
{
    ...
    txtFile << std::setw(definitions().formula().size() + 15) << definitions().formula()
            << std::setw(15);
    if (definitions().isResponse())
