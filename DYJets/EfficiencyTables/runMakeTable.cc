...
void makeEfficiencyTable(string workingPoint)
{
    ...
    if (workingPoint.find("Iso") == string::npos)
        cout << "\\multicolumn{5}{c}{Scale Factors for Muons Tight ID} \\\\ \\hline" << endl;
    else if (workingPoint.find("<02") != string::npos)
        cout << "\\multicolumn{5}{c}{Scale Factors for Tight Muons CombRelIsodBeta $< 0.2$ "
                "(R=0.4)} \\\\ \\hline"
             << endl;
    else if (workingPoint.find("<012") != string::npos)
        cout << "\\multicolumn{5}{c}{Scale Factors for Tight Muons CombRelIsodBeta $< 0.12$ "
                "(R=0.4)} \\\\ \\hline"
             << endl;
    cout << "$p_T$  $\\backslash$ $|\\eta|$"
         << " & $0 < |\\eta| \\leq 0.9$ "
         << "& $0.9 < |\\eta| \\leq 1.2$ "
         << "& $1.2 < | \\eta | \\leq 2.1$ "
         << "& $2.1 < |\\eta| \\leq 2.4$ \\\\ \\hline" << endl;
